import { d as dc, O as Oe, v as ft, X as Xo, y, B as or } from "./indexhtml-Crv9HnRQ.js";
class V extends dc {
  constructor() {
    super(...arguments), this.eventBusRemovers = [], this.messageHandlers = {}, this.handleESC = (t) => {
      const e = Oe.getPanelByTag(this.tagName);
      ft().appInteractable && e && !e.individual || t.key === "Escape" && Xo(this);
    };
  }
  /**
  ...
  onEventBus(t, e) {
    this.eventBusRemovers.push(y.on(t, e));
  }
  connectedCallback() {
  ...
  handleMessage(t) {
    return this.messageHandlers[t.command] ? (this.messageHandlers[t.command].call(this, t), true) : false;
  }
  /**
  ...
  requestLayoutUpdate() {
    ...
    if (Oe.positionUpdatedManually(t))
      return Promise.resolve();
    const e = Oe.getPanelByTag(t);
    return e ? new Promise((r) => {
      requestAnimationFrame(() => {
        const o = or(this, "vaadin-dialog");
        if (!o) {
          const x = or(this, "vaadin-popover");
        ...
        const y2 = e.position, H = Number.parseFloat(o.getAttribute("top") ?? ""), R = Number.parseFloat(o.getAttribute("left") ?? ""), U = y2?.top ?? (Number.isNaN(H) ? 0 : H), _ = y2?.left ?? (Number.isNaN(R) ? 0 : R), B = document.querySelector("copilot-main")?.shadowRoot?.querySelector(`copilot-toolbar #${t}-toolbar-btn`);
        let g = U, w = _;
        if (B) {
        ...
        g = Math.max(a, Math.min(g, window.innerHeight - a - u)), w = Math.max(a, Math.min(w, window.innerWidth - a - f)), Oe.updatePanel(
          t,
          {
            position: {
              top: g,
